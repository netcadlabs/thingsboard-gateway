...
class TBGatewayService(Thread):
                ...
                try:
                    ...
                    with open(config_file_path, 'r', encoding="UTF-8") as conf_file:
                        try:
                            ...
                            connector_conf["custom_config"] = self._load_custom_conf(connector)
                        except Exception as e:
    ...
    def _load_custom_conf(self, connector):
        connector_custom_conf = {}
        custom_config_file_path = self._config_dir + connector['type'] + "_custom.json"
        if path.isfile(custom_config_file_path):
            with open(custom_config_file_path, 'r', encoding="UTF-8") as custom_conf_file:
                try:
                    connector_custom_conf = load(custom_conf_file)
                except Exception as e:
    ...
    def add_device(self, device_name, content, device_type=None, device_capabilities=None):
